...
template <typename T>
std::ifstream& operator >> (std::ifstream& infile, std::string filename, Node<T>* root)
{
	return root.infile(filename, infile);
} 
...
template <typename T>
void BinarySearchTree<T>::infile(std::string filename, std::ifstream infile)
{
	infile.open(filename);
	if (!infile)
		throw std::logic_error("The file isn't find");
